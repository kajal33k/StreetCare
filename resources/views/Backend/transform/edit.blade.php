<x-layouts.app :title="__('Transform')">
    ...
    <div class="max-w-xl mx-auto py-10 px-4">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit transform</h1>

        <form action="{{ route('backend.transform.update', $transform) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" id="title" name="title"
                       value="{{ old('title', $transform->title) }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full border border-gray-300 px-4 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          required>{{ old('description', $transform->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="mb-3">
                    @if($difference->image)
                        <img src="{{ asset('storage/' . $transform->image) }}"
                             alt="Current Image"

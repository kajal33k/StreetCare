<x-layouts.app :title="__('Transform')">
    ...
    <div class="max-w-xl mx-auto py-10">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Create Transform</h1>

        <form action="{{ route('backend.transform.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-transparent p-6 rounded-lg shadow-sm border border-gray-200">
            @csrf
            ...
            <div>
                ...
                <input type="file" id="image" name="image"
                       class="w-full text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-white file:bg-indigo-600 hover:file:bg-indigo-700">
                @error('image')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
